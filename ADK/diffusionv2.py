...
# ---------------------------------------------------------------------------
# PMP inference-time noise control (Wasserstein PMP controller).
#
# Controlled reverse kernel on the late-reverse window t in [0, t_w]:
#     nu(t) = alpha * g(t)
#     injected noise std :  g(t)  ->  nu(t)
#
# alpha = 1.0 (default) reproduces the stock Boltz-2 sampler EXACTLY
# (the control block below is skipped). alpha > 1 boosts late-window
# noise (escape from premature contraction); 0 < alpha < 1 damps it.
#
# In EDM coordinates the per-step reference noise variance g^2 is the
# stock churn variance when churn is active (sigma > gamma_min), and the
# variance-exploding per-step increment sigma_tm^2 - sigma_t^2 (the EDM
# analog of DDPM's beta_t) when the stock sampler is deterministic
# (sigma <= gamma_min), which covers the whole default late window.
#
# Note on the drift: the DDPM form of the controller also rescales the
# score coefficient, g^2 -> (g^2 + nu^2)/2. That correction compensates
# the injected noise within a fine-grained Euler-Maruyama step. Boltz's
# churn+Euler sampler is instead a splitting scheme whose ODE descent
# traverses the FULL gap t_hat -> sigma_t and so removes exactly the
# injected noise by construction; rescaling that descent overshoots the
# noise schedule and destroys structures (verified: alpha=3, t_w=0.3 with
# drift scaling unfolded all 100 models, mean RMSD ~25 A). The marginal
# schedule is preserved by the unscaled descent, so the faithful port is
# noise-magnitude control alone. Set NU_DRIFT=1 to restore the literal
# DDPM drift factor for comparison.
#
# Configure via environment variables:
#     NU_ALPHA  (default 1.0 = stock), NU_TW (default 0.2),
#     NU_DRIFT  (default 0)
# ---------------------------------------------------------------------------
_NU_ALPHA = float(os.environ.get("NU_ALPHA", "1.0"))
_NU_TW = float(os.environ.get("NU_TW", "0.2"))
_NU_DRIFT = bool(int(os.environ.get("NU_DRIFT", "0")))
_NU_PRINTED = False
    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        for step_idx, (sigma_tm, sigma_t, gamma) in enumerate(sigmas_and_gammas):
            ...
            # --- PMP inference-time noise control (two-line modification) ---
            # Forward-time coordinate t in [0, 1]; t = 0 is the clean sample,
            # so the late-reverse window [0, t_w] covers the last steps.
            t_norm = 1.0 - (step_idx / num_sampling_steps)
            if _NU_ALPHA != 1.0 and t_norm <= _NU_TW and sigma_t > 0.0:
                global _NU_PRINTED
                if not _NU_PRINTED:
                    print(
                        f"[nu-control] alpha={_NU_ALPHA}, window=[0, {_NU_TW}] "
                        f"(active on ~{int(_NU_TW * num_sampling_steps)} of "
                        f"{num_sampling_steps} steps, drift={'on' if _NU_DRIFT else 'off'})"
                    )
                    _NU_PRINTED = True
                # Reference per-step noise variance g(t)^2: stock churn
                # variance when active; VE increment (EDM analog of beta_t)
                # where the stock sampler is deterministic.
                g2 = (
                    noise_var
                    if noise_var > 0.0
                    else max(sigma_tm**2 - sigma_t**2, 1e-12)
                )
                # Damping semantics: alpha < 1 can only remove noise that the
                # stock sampler actually injects. Where stock is deterministic
                # (noise_var == 0) there is nothing to damp, so the step is
                # left untouched instead of injecting alpha*g_ref > 0.
                if _NU_ALPHA < 1.0 and noise_var <= 0.0:
                    pass  # stock deterministic step: nothing to damp
                else:
                    nu2 = (_NU_ALPHA**2) * g2  # nu(t) = alpha * g(t)
                    # Injected noise std: g(t) -> nu(t). Injecting nu^2 raises
                    # the noise level to t_hat_c = sqrt(sigma_tm^2 + nu^2); the
                    # denoiser is queried at this true level below, and the
                    # (unscaled) ODE descent then traverses the full gap
                    # t_hat_c -> sigma_t, keeping the marginal on schedule.
                    t_hat = sqrt(sigma_tm**2 + nu2)
                    noise_var = nu2
                    if _NU_DRIFT:
                        # Optional literal DDPM drift factor (g^2+nu^2)/(2g^2).
                        # OFF by default: with the full-gap ODE descent it
                        # overshoots the schedule (see header note).
                        drift_scale = (g2 + nu2) / (2.0 * g2)
            # ----------------------------------------------------------------
            ...
            atom_coords_next = (
                atom_coords_noisy
                + drift_scale * step_scale * (sigma_t - t_hat) * denoised_over_sigma
            )
