...
Figures for Section 4.4 (d = 32 only), split so that each panel is saved
as its own standalone figure.

Outputs (300 DPI, written to ../figs/):
  fig_convergence_boost.png       Picard residuals for the boost lambda sweep
    fig_convergence_compare.png     Picard residual comparison for positive-delta cases
  fig_schedule_boost.png          optimal vs reference schedule, boost
  fig_schedule_damp.png           optimal vs reference schedule, damp
...
def fig_convergence_boost():
    ...
    fig.tight_layout()
    outpath = os.path.join(FIGDIR, "fig_convergence_boost.png")
    fig.savefig(outpath, dpi=300)
    plt.close(fig)
    return rates


def fig_convergence_compare():
    zd = np.load("res/picard_damp_l0.8.npz")
    zd16 = np.load("res/picard_damp_l16.npz")
    fig, ax = plt.subplots(figsize=(4.8, 3.4))
    for z, c, lab in [(zd, C_RED, r"damp ($\bar\delta=+0.1$, $\lambda=0.8$)"),
                      (zd16, C_BROWN, r"damp ($\bar\delta=+0.1$, $\lambda=16$)")]:
        r = z["res"]
        ax.semilogy(np.arange(len(r)), r, "o-", ms=3.5, lw=1.4, color=c, label=lab)
    ax.set_xlabel("iteration $n$")
    ax.set_ylabel(r"$\|\nu_{n+1}-\nu_n\|_{L^2}$")
    ax.set_title(r"positive-$\bar\delta$ convergence")
    ax.legend(fontsize=8, frameon=False)
    fig.tight_layout()
    outpath = os.path.join(FIGDIR, "fig_convergence_compare.png")
    fig.savefig(outpath, dpi=300)
    plt.close(fig)


def _schedule_figure(tag, col, title, outname):
    ...
    ax.set_ylabel(r"schedule")
    ax.set_title(title)
    ax.legend(fontsize=8, frameon=False, loc="upper left")
    fig.tight_layout()
    outpath = os.path.join(FIGDIR, outname)
    fig.savefig(outpath, dpi=300)
    plt.close(fig)


def fig_schedule_boost():
    _schedule_figure(
        "boost_l0.8",
        C_BLUE,
        r"boost scenario ($\bar\delta=-0.1$)",
        "fig_schedule_boost.png",
    )


def fig_schedule_damp():
    _schedule_figure(
        "damp_l16",
        C_RED,
        r"damp scenario ($\bar\delta=+0.1$)",
        "fig_schedule_damp.png",
    )
...
def fig_kindependence():
    ...
    ax.fill_between([Ks[0], Ks[-1]], mean * 0.95, mean * 1.05, color="1",
                    alpha=0.12)
...
if __name__ == "__main__":
    rates = fig_convergence_boost()
    fig_convergence_compare()
    fig_schedule_boost()
    fig_schedule_damp()
