...
def s_ctrl_gmm(x, centers, w, v):
    ...
    r = np.exp(lg)
    r /= r.sum(axis=1, keepdims=True)
    pulls = -(x[:, None, :] - centers[None, :, :]) / v[None, :, None]
    return (r[:, :, None] * pulls).sum(axis=1), r
...
def grad_psi_mix(X, centers, w, v, coef):
    ...
    d2pulls = -(X[:, None, :] - centers[None, :, :]) / v[None, :, None]
    F = np.concatenate([np.ones((N, 1)), X, X**2], axis=1)
    q = F @ coef.T                                             # (N,K)
    # grad r_k = r_k (grad log N_k - sum_l r_l grad log N_l) = r_k (pull_k - s)
    grad_r = r[:, :, None] * (d2pulls - s[:, None, :])         # (N,K,d)
    grad_q = coef[None, :, 1:1 + d] + 2.0 * coef[None, :, 1 + d:] * X[:, None, :]
    return (q[:, :, None] * grad_r + r[:, :, None] * grad_q).sum(axis=1)
